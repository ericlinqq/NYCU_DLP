...
if __name__ == "__main__":
    ...
    parser.add_argument('--gn_real_sigma', type=float, help="standard deviation for gaussian noise added to real data", default=0.0)
    parser.add_argument('--nn_sigma_enc', type=float, help="size of relative standard deviation of in-layer gaussian noise in the encoder", default=0.0)
    parser.add_argument('--nn_sigma_dec', type=float, help="size of relative standard deviation of in-layer gaussian noise in the decoder", default=0.0)
    parser.add_argument('--nn_gn_rel', type=bool, help="if specified, in-layer gaussian noise std will be a learned parameter", default=True)
    parser.add_argument('--p_enc_s', type=float, help="dropout probability after the first layer in the encoder", default=0.0)
    parser.add_argument('--p_enc_e', type=float, help="dropout probability after the last layer in the encoder", default=0.0)
    parser.add_argument('--p_dec_s', type=float, help="dropout probability after the first layer in the decoder", default=0.0)
    parser.add_argument('--p_dec_e', type=float, help="dropout probability after the last layer in the decoder", default=0.0)
    parser.add_argument('--drop_dis', type=bool, help="if specified, dropout will be disable in the other module during training", default=False)
    parser.add_argument('--p_augment', type=float, help="data augmentation probability in each augmentation layer", default=0.0)
    parser.add_argument('--p_ls_enc', type=float, help="label switch percentage per batch in encoder training", default=0.0)
    parser.add_argument('--p_ls_dec', type=float, help="label switch percentage per batch in decoder training", default=0.0)
    ...
    train_soft_intro_vae(dataset=args.dataset, z_dim=args.z_dim, batch_size=args.batch_size, num_workers=0,
                         ...
                         test_iter=1000, with_fid=args.fid, p_enc_s=args.p_enc_s, p_enc_e=args.p_enc_e, p_dec_s=args.p_dec_s,
                         p_dec_e=args.p_dec_e, nn_sigma_enc=args.nn_sigma_enc, nn_sigma_dec=args.nn_sigma_dec, 
                         nn_gn_rel=args.nn_gn_rel, p_augment=args.p_augment)
