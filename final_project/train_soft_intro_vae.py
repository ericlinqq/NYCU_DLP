...
def train_soft_intro_vae(dataset='cifar10', z_dim=128, lr_e=2e-4, lr_d=2e-4, batch_size=128, num_workers=4,
                         ...
                         device=torch.device("cpu"), num_row=8, gamma_r=1e-8, with_fid=False, p_enc_s=0,
                         p_enc_e=0, p_dec_s=0, p_dec_e=0, nn_sigma_enc=0, nn_sigma_dec=0, nn_gn_rel=True,
                         p_augment=0):
    """
    ...
    model = SoftIntroVAE(cdim=ch, zdim=z_dim, channels=channels, image_size=image_size, p_enc_s=p_enc_s,
    p_enc_e=p_enc_e, p_dec_s=p_dec_s, p_dec_e=p_dec_e, nn_sigma_enc=nn_sigma_enc, nn_sigma_dec=nn_sigma_dec,
    nn_gn_rel=nn_gn_rel, p_augment=p_augment).to(device)
