...
```  
  ...
  Note that I also train the model with **WGAN-GP** loss function, the WGAN suggests that not using the **sigmoid** function in the last layer of the discriminator, and since the gradient penalty is performed on each sample seperately, **batch normaliztion** in the discriminator **should be removed**, and using such as layer, weight or instance normalization.  
...
![WGAN new_test](https://i.imgur.com/yFUYwcQ.png)  
  ...
  During the testing, I found that WGAN-GP is very sensitive to the random noise while GAN is relevantly stable.  
